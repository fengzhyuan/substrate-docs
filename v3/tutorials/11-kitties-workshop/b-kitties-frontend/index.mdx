...
1.  Proceed by pasting in the following code snippet:
    ...
    export default function Kitties (props) {
      const { state: {api, keyring} } = useSubstrate();
...
1.  Now, all that's left to do in order for our component to listen for changes in our node's storage is to pass in `subscribeKittyCnt` and `subscribeKitties` to React's `useEffect` function.
